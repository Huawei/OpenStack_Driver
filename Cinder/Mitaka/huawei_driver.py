...
class HuaweiBaseDriver(driver.VolumeDriver):
    ...
    def update_support_capability(self, stats):
        ...
        feature_dict = {
            ...
            'dedup': ['true', 'false'] if huawei_utils.check_feature_available(
                feature_status, constants.DEDUP_FEATURES) else ['false'],
            'compression': (
                ['true', 'false'] if huawei_utils.check_feature_available(
    ...
    def _add_extend_type_to_volume(self, volume, volume_type, opts, lun_params,
        ...
        if opts.get('hypermetro') == 'true':
            ...
            metro_info = metro.create_hypermetro(lun_id, lun_params, is_sync)
    ...
    def create_consistencygroup(self, context, group):
        ...
        if self._cg_volume_type_support(cg_opts, 'hypermetro'):
            if not self.support_func.get('hypermetro'):
                msg = (_("Can't create hypermetro consistency group, local or "
                         "remote array not support hypermetro consistentgroup,"
                         " group id: %(group_id)s.")
                       % {"group_id": group.id})
                LOG.error(msg)
                raise exception.VolumeBackendAPIException(data=msg)
    ...
    def delete_consistencygroup(self, context, group, volumes):
        ...
        for volume in volumes:
            ...
            try:
                ...
                volume_model_update['status'] = 'error_deleting'
            else:
