...
class HuaweiConf(object):
    ...
    def _encode_authentication(self, tree, xml_root):
        ...
        if (name_node is not None
            ...
            name_node.text = node_start_text + base64.b64encode(name_node.text)
            need_encode = True
        if (pwd_node is not None
                and not pwd_node.text.startswith(node_start_text)):
            pwd_node.text = node_start_text + base64.b64encode(pwd_node.text)
            need_encode = True
        if (vstore_node is not None
                and not vstore_node.text.startswith(node_start_text)):
            vstore_node.text = node_start_text + base64.b64encode(vstore_node.text)
            need_encode = True
