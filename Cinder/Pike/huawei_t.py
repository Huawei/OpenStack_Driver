...
class HuaweiTISCSIDriver(driver.ISCSIDriver):
    ...
    def terminate_connection(self, volume, connector, **kwargs):
        ...
        if ('host' not in connector) or ('initiator' not in connector):
            LOG.info("terminate_connection: delete or force delete.")
            return
        ...
        if not lun_id:
            LOG.warning("Volume %s not exists on the array.", volume['id'])
        ...
        info = {'driver_volume_type': 'iSCSI',
                'data': {'iqn': iqn}}
        LOG.info('terminate_connection, return data is: %s.', info)
        return info
    ...
    def _remove_iscsi_port(self, hostid, initiator):
        ...
        if port_info:
            ...
            LOG.warning('_remove_iscsi_port: iSCSI port was not found '
...
class HuaweiTFCDriver(driver.FibreChannelDriver):
    ...
    def _remove_fc_ports(self, hostid, wwns):
        ...
        if port_info:
            ...
            LOG.warning('_remove_fc_ports: FC port was not found '
                        'on host %(hostid)s.', {'hostid': hostid})
