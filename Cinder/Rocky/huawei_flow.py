...
class CreateHyperMetroTask(task.Task):
    ...
    def execute(self, volume, lun_id, lun_info, opts):
        ...
        if opts.get('hypermetro') and not hypermetro_id:
            ...
            hypermetro_id = self.hypermetro.create_hypermetro(
                lun_id, lun_params)
        elif not opts.get('hypermetro') and hypermetro_id:
            hypermetro_id = None
    ...
    def revert(self, result, **kwargs):
        if isinstance(result, failure.Failure):
            return
        if result:
            self.hypermetro.delete_hypermetro(result)
...
class DeleteHyperMetroTask(task.Task):
    ...
    def execute(self, volume, opts=None):
        metadata = huawei_utils.get_volume_private_data(volume)
        hypermetro_id = metadata.get('hypermetro_id')
        if (not opts or not opts.get('hypermetro')) and hypermetro_id:
            self.hypermetro.delete_hypermetro(hypermetro_id, volume)
...
class ExtendHyperMetroTask(task.Task):
    ...
    def execute(self, volume, new_size):
        ...
        hypermetro_id = metadata.get('hypermetro_id')
        if hypermetro_id:
            self.hypermetro.extend_hypermetro(hypermetro_id, new_size)
...
class GetISCSIConnectionTask(task.Task):
    ...
    def _get_config_target_ips(self, initiator):
    ...
    def execute(self, connector):
        ...
        config_ips = self._get_config_target_ips(initiator)
        ...
        return target_ips, target_iqns, target_eths
...
class AddISCSIInitiatorTask(task.Task):
    ...
    def _get_chap_info(self, initiator):
        ini = self.iscsi_info['initiators'].get(initiator)
        config = ini if ini else self.iscsi_info
    ...
    def _get_alua_info(self, initiator):
        ini = self.iscsi_info['initiators'].get(initiator)
        config = ini if ini else self.iscsi_info
    ...
    def execute(self, connector, host_id):
        ...
        alua_info = self._get_alua_info(initiator)
        self.client.associate_iscsi_initiator_to_host(
            initiator, host_id, alua_info)

        chap_info = self._get_chap_info(initiator)
...
class AddFCInitiatorTask(task.Task):
    ...
    def _get_alua_info(self, initiator):
        ini = self.fc_info['initiators'].get(initiator)
        config = ini if ini else self.fc_info
    ...
    def execute(self, host_id, ini_tgt_map):
        for ini in ini_tgt_map:
            ...
            alua_info = self._get_alua_info(ini)
...
def migrate_volume(volume, host, local_cli, feature_support):
    store_spec = {'volume': volume}
...
def retype(volume, new_opts, local_cli, hypermetro_rmt_cli,
    ...
    work_flow.add(
        LunOptsCheckTask(local_cli, feature_support, new_opts),
        CheckLunExistTask(local_cli),
