...
class SmartX(object):
    ...
    def get_smartx_extra_specs_opts(self, opts):
        ...
        opts = self.get_capabilities_opts(opts, 'huawei_share_privilege')
    ...
    def get_capabilities_opts(self, opts, key):
        if strutils.bool_from_string(opts[key], strict=True):
            opts[key] = True
        else:
            opts[key] = False
    ...
    def get_smartprovisioning_opts(self, opts):
        ...
        if (thin_provision is None or
                strutils.bool_from_string(thin_provision, strict=True)):
    ...
    def get_controller_opts(self, opts):
        if strutils.bool_from_string(opts['huawei_controller'], strict=True):
    ...
    def get_smartcache_opts(self, opts):
        if strutils.bool_from_string(opts['huawei_smartcache'], strict=True):
            if not opts['cachename']:
                raise exception.InvalidInput(
    ...
    def get_sectorsize_opts(self, opts):
        value = None
        if (opts.get('huawei_sectorsize') and
                strutils.bool_from_string(opts['huawei_sectorsize'],
                                          strict=True)):
    ...
    def get_qos_opts(self, opts):
        qos = {}
        if not strutils.bool_from_string(opts.get('qos'), strict=True):
            return
