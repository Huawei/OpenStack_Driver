...
class RestClient(object):
    ...
    def get_inband_pools(self):
        pools = self.get_all_pools()

        def is_inband_pool(pool):
            return pool["NAME"] in self.storage_pools
        return filter(is_inband_pool, pools)
    ...
    def get_lun_info_by_name(self, name):
        ...
        result = self.call(url, None, "GET")
        self._assert_rest_result(result, _('Get lun info by name error.'))
        if result.get('data'):
    ...
    def get_init_targ_map(self, wwns, is_get_manage_tgt=False):
        init_targ_map = {}
        tgt_port_wwns = []
        for wwn in wwns:
        ...
        if not tgt_port_wwns and not is_get_manage_tgt:
    ...
    def get_all_luns(self, pool_id):
        count = self.get_lun_count(pool_id)
        for i in range(count // constants.MAX_QUERY_LUN_COUNT + 1):
            begin = i * constants.MAX_QUERY_LUN_COUNT
            query_range = (begin, begin + constants.MAX_QUERY_LUN_COUNT)
            result = self.get_luns_page(query_range, pool_id)
            luns = result.get('data', [])
            for lun in luns:
                yield lun

    def get_luns_page(self, query_range, pool_id):
        url = "/lun?filter=PARENTID::%s&range=[%d-%d]" % (
            pool_id, query_range[0], query_range[1])
        result = self.call(url, None, "GET", filter_flag=True)
        self._assert_rest_result(result, _('Get volume by name error.'))
        return result

    def get_lun_count(self, pool_id):
        url = "/lun/count?filter=PARENTID::%s" % pool_id
        result = self.call(url, None, "GET", filter_flag=True)
        msg = _('Get volume error.')
        self._assert_rest_result(result, msg)
        self._assert_data_in_result(result, msg)
    ...
    def get_fc_initiator(self, initiator):
        url = '/fc_initiator?filter=ID::%s' % initiator
        result = self.call(url, None, "GET")
        self._assert_rest_result(result, _('Get fc initiator error.'))
    ...
    def get_host_by_id(self, host_id):
        url = "/host/%s" % host_id
        result = self.call(url, None, "GET")
        self._assert_rest_result(result, _('Get host error.'))

        return result['data']

    def get_fc_ports_count(self):
        url = "/fc_port/count"
        result = self.call(url, None, "GET")

        msg = _('Get fc port count error.')
        self._assert_rest_result(result, msg)
        self._assert_data_in_result(result, msg)
    ...
    def get_all_fc_ports(self):
        fc_ports = []
        fc_port_count = self.get_fc_ports_count()
        if fc_port_count <= 0:
            return fc_ports

        for i in range((fc_port_count - 1) // constants.MAX_QUERY_COUNT + 1):
            url = '/fc_port?range=[%d-%d]' % (
                i * constants.MAX_QUERY_COUNT,
                (i + 1) * constants.MAX_QUERY_COUNT)
            result = self.call(url, None, "GET", filter_flag=True)

            msg = _('Get FC ports info from array error.')
            self._assert_rest_result(result, msg)

            if "data" in result:
                for item in result["data"]:
                    fc_ports.append(item)
        return fc_ports
    ...
    def get_fc_initiator_info(self, ininame):
        """Check whether the fc initiator is already added on the array."""
        url = "/fc_initiator/" + ininame
        result = self.call(url, None, "GET")
        error_code = result['error']['code']
    ...
    def get_host_lun_info(self, host_id):
        url = ("/lun/associate?ASSOCIATEOBJTYPE=21"
               "&ASSOCIATEOBJID=%s" % host_id)
        result = self.call(url, None, "GET")
        self._assert_rest_result(result, _('Find host lun info error.'))
        return result.get("data")

    def get_lun_map_host_info(self, lun_id):
        url = "/host/associate?ASSOCIATEOBJTYPE=11&ASSOCIATEOBJID=" + lun_id
        result = self.call(url, None, "GET")
        self._assert_rest_result(result, _('Find lun map host info error.'))
        return result.get("data")

    def get_hypermetro_by_local_lun_id(self, lun_id):
    ...
    def get_lun_info_by_wwn(self, wwn):
        url = "/lun?filter=WWN::" + wwn
        result = self.call(url, None, "GET")
        self._assert_rest_result(result, _('Find lun info by wwn error.'))
        if result.get('data'):
            return result['data'][0]
        return {}
