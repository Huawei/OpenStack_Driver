...
from powervc_cinder.volume.drivers.ibm_storage.common_driver import \
...
class HuaweiBaseDriver(driver.VolumeDriver,
    ...
    @staticmethod
    def _is_feature_support(feature_status, default_license, config_license):
        feature_tuple = tuple(set(default_license) | set(config_license))
        return huawei_utils.check_feature_available(
                feature_status, feature_tuple)
    ...
    def update_support_capability(self, stats):
        ...
        feature_dict = {
            ...
            'dedup': [self._is_feature_support(
                feature_status, constants.DEDUP_FEATURES,
                self.configuration.dedup_license), False],
            'compression': [self._is_feature_support(
                feature_status, constants.COMPRESSION_FEATURES,
                self.configuration.compression_license), False],
        ...
        for pool in stats['pools']:
            ...
            if self.configuration.san_product in \
                    constants.DORADO_V6_AND_V6_PRODUCT:
    ...
    def _create_volume_from_src(self, volume, src_obj, src_type, lun_params,
        ...
        if strutils.bool_from_string(metadata.get('fastclone')):
    ...
    def _update_restricted_metadata(self, volume_id, metadata):
    ...
    def _common_create_volume(self, volume, src_obj=None, src_type=None,
        ...
        metadata = {RESTRICTED_METADATA_VDISK_ID_KEY: lun_info['WWN'],
                    RESTRICTED_METADATA_VDISK_NAME_KEY: lun_info['WWN'],
                    RESTRICTED_METADATA_VDISK_UID_KEY: lun_info['WWN'],
                    }
        self._update_restricted_metadata(volume.id, metadata)
    ...
    def update_migrated_volume(self, ctxt, volume, new_volume,
        ...
        original_name = huawei_utils.get_lun_name(self.client, volume)
    ...
    def _migrate_volume(self, volume, host, new_type=None):
        ...
        for key in list(lun_params):
            if lun_params.get(key) == '--':
                lun_params.pop(key, None)
    ...
    def _check_volume_exist_on_array(self, volume, action, local=True):
        ...
        # For manage scenario remote name would not same with local name
        hypermetro_id = huawei_utils.get_hypermetro_id(volume)
        if not local and hypermetro_id:
            metro_info = self.rmt_client.get_hypermetro_by_id(hypermetro_id)
            lun_id = metro_info['LOCALOBJID']
            lun_wwn = None
        else:
            # try to find LUN ID from volume.
    ...
    def _extend_hypermetro_volume(self, volume, new_size):
        lun_name = huawei_utils.get_lun_name(self.client, volume, True)
    ...
    def remove_host_with_check(self, host_id, client):
        wwns_in_host = (
            client.get_host_fc_initiators(host_id))
        iqns_in_host = (
            client.get_host_iscsi_initiators(host_id))
        if not (wwns_in_host or iqns_in_host or
                client.is_host_associated_to_hostgroup(host_id)):
    ...
    def _get_all_fc_ports(self):
        fc_ports = self.client.get_all_fc_ports()

        if self.rmt_client:
            try:
                rmt_fc_ports = self.rmt_client.get_all_fc_ports()
                fc_ports.extend(rmt_fc_ports)
            except Exception:
                LOG.warning("Get HyperMetro remote storage FC port failed, "
                            "ignore it")

        if self.replica_client:
            try:
                rmt_fc_ports = self.replica_client.get_all_fc_ports()
                fc_ports.extend(rmt_fc_ports)
            except Exception:
                LOG.warning("Get Replication remote storage FC port failed, "
                            "ignore it")
    ...
    def discover_storage_ports(self, details=False, fabric_map=False,
                               all_ports=False):
        """Implementation for the discovery_driver method."""
        LOG.debug("Discover storage ports: %(details)s, %(fabric_map)s, "
                  "%(all_ports)s"
                  % {"details": details,
                     "fabric_map": fabric_map,
                     "all_ports": all_ports})
        ports = {}
        fc_ports = self._get_all_fc_ports()
        for fc_port in fc_ports:
            LOG.debug("Port is: %s" % fc_port)
            if (not all_ports and fc_port.get("RUNNINGSTATUS") not in
                    constants.FC_PORT_RUNNING_NORMAL):
                continue
            port = {"wwpn": str(fc_port["WWN"]),
                    "status": "online",
                    "location_info": str(fc_port["LOCATION"]),
                    }
            if fc_port.get("HEALTHSTATUS") != constants.STATUS_HEALTH:
                port["status"] = "down"
            if details:
                if fc_port["RUNSPEED"] != constants.FC_PORT_SPEED_ERROR:
                    port["speed"] = "%s Gbs" % (
                            int(fc_port["RUNSPEED"]) // 1000)
                else:
                    port["speed"] = "unknown"
                port["conf_mode"] = fc_port["FCCONFMODE"]
                port["port_switch"] = fc_port["PORTSWITCH"]
            if all_ports:
                port['host_io_permitted'] = (fc_port.get("RUNNINGSTATUS") ==
                                             constants.FC_PORT_STATUS_LINK_UP)
            ports[port["wwpn"]] = port
        if fabric_map:
            ports = self._add_fabric_mapping(ports)
        return ports

    def _get_tgt_wwns_from_storage(self, client, source_wwn):
        tgt_port_wwns, init_targ_map = client.get_init_targ_map(source_wwn,
                                                                is_get_manage_tgt=True)
        return tgt_port_wwns

    def _get_volume_connection_info(self, lun_map_host_info, lun_info):
        connect_info = {}
        itl_list = []
        for host_info in lun_map_host_info:
            lun_map_host_id = host_info.get('ID')
            lun_map_host_name = host_info.get('NAME')
            wwns_in_host = self.client.get_host_fc_initiators(
                lun_map_host_id)
            LOG.info("the wwns_in_host is %s", wwns_in_host)

            source_wwn = huawei_utils.convert_connector_wwns(wwns_in_host)
            LOG.info("the source_wwn is %s", source_wwn)

            tgt_wwns = self._get_tgt_wwns_from_storage(self.client, source_wwn)
            tgt_wwns_remote = []
            hypermetro_pair = self.client.get_hypermetro_by_local_lun_id(lun_info["ID"])
            if self._is_hypermetro_enabled(hypermetro_pair):
                tgt_wwns_remote = self._get_tgt_wwns_from_storage(
                    self.rmt_client, source_wwn)

            tgt_wwns += tgt_wwns_remote

            host_lun_id = self.client.get_host_lun_id(lun_map_host_id, lun_info["ID"])
            connect_object = {
                'source_wwn': source_wwn,
                'target_lun': host_lun_id,
                'host': lun_map_host_name,
                'target_wwn': tgt_wwns
            }
            LOG.info("get the connect_object is %s", connect_object)
            connect_info[connect_object['host']] = connect_object
            itl_obj = discovery_driver.ITLObject(
                source_wwn, tgt_wwns, host_lun_id,
                vios_host=lun_map_host_name)
            itl_list.append(itl_obj)

        return connect_info, itl_list

    def _get_vol_info_for_manage(self, lun_info):
        if lun_info.get("ID") is None:
            return {}
        lun_map_host_info = self.client.get_lun_map_host_info(
            lun_info["ID"])
        vol = self._build_volume_info(lun_info)
        if not lun_map_host_info:
            return vol
        connect_info, itl_list = self._get_volume_connection_info(
            lun_map_host_info, lun_info)
        vol['connection_info'] = connect_info
        vol['itl_list'] = itl_list
        for key, value in vol['restricted_metadata'].items():
            vol['restricted_metadata'][key] = value.upper()
        return vol

    def _get_vols_info_from_vol_refs(self, vol_refs):
        vols = []
        lun_id_filter_list = []
        for vol in vol_refs:
            wwn = vol.get("uid")
            if wwn is None:
                continue
            lun_info = self.client.get_lun_info_by_wwn(wwn.lower())
            lun_id = lun_info.get("ID")
            if lun_id is None or lun_id in lun_id_filter_list:
                continue
            lun_id_filter_list.append(lun_id)

            vol = self._get_vol_info_for_manage(lun_info)
            if not vol:
                continue
            vols.append(vol)
        return vols, lun_id_filter_list

    def _get_vols_from_exist_host_info(self, exist_host_info, lun_id_filter_list):
        vols = []
        for host_id in exist_host_info:
            lun_in_host = self.client.get_host_lun_info(host_id)
            for lun_info in lun_in_host:
                lun_id = lun_info.get('ID')
                if lun_id is None or lun_id in lun_id_filter_list:
                    continue
                lun_id_filter_list.append(lun_id)
    ...
    def _get_vols_from_filter(self, filter_set, vol_refs):
        vols = []
        exist_host_info = {}
        for wwn in filter_set:
            ini = self.client.get_fc_initiator_info(wwn)
            if ini and ini['ISFREE'] == 'false':
                exist_host_info[ini['PARENTID']] = ini['PARENTNAME']

        if not exist_host_info and not vol_refs:
            return vols

        lun_id_filter_list = []
        if vol_refs:
            vols_from_vol_refs, lun_id_filter_list = (
                self._get_vols_info_from_vol_refs(vol_refs))
            vols += vols_from_vol_refs

        vols_from_exist_host = self._get_vols_from_exist_host_info(
            exist_host_info, lun_id_filter_list)
        vols += vols_from_exist_host
        return vols
    ...
    def get_volume_info(self, vol_refs, filter_set):
        ...
        the following attributes:
            ...
            itl_list:  A dictionary containing vscsi itl information
            connection_info: Host and storage wppns for volume connections
            k2udid:    Passed in from HMC for vscsi volumes
        ...
        if vol_refs or filter_set:
            LOG.info("Filter Set %(filter_set)s "
                     "vol_refs : %(vol_rf)s",
                     {"filter_set": filter_set,
                      "vol_rf": vol_refs})
            vols = self._get_vols_from_filter(filter_set, vol_refs)
            return vols

        vols = []
        pools = self.client.get_inband_pools()
        for pool in pools:
            for lun in self.client.get_all_luns(pool["ID"]):
                vol = self._build_volume_info(lun)
                vols.append(vol)
        return vols

    def _build_volume_info(self, lun):
        size = int(lun["CAPACITY"]) // constants.CAPACITY_UNIT
        ...
        volume = {
            ...
            'is_mapped': strutils.bool_from_string(lun['EXPOSEDTOINITIATOR']),
            'restricted_metadata': {
                'vdisk_id': lun['WWN'],
                'vdisk_name': lun['WWN'],
                'vdisk_uid': lun['WWN'],
            ...
            'provider_location': lun['ID'],
        }

        self._check_lun_info(volume, lun)
        self._check_volume_status(volume)
        self._check_in_use(volume)
        return volume

    def _check_lun_info(self, volume, lun):

        def check_health_status(lun):
            return lun["HEALTHSTATUS"] == constants.LUN_HEALTH_STATUS_NORMAL

        def check_running_status(lun):
            return lun['RUNNINGSTATUS'] == constants.LUN_RUNNING_STATUS_ONLINE

        def check_snapshot(lun):
            # The volume with snapshots is not permitted to managed
            snap_ids = lun.get('SNAPSHOTIDS')
            return not snap_ids or snap_ids == '[]'

        def check_function_type(lun):
            # Dorado V6 scenario: Only volume can be managed
            return lun.get('functionType', '1') == constants.FUNCTION_TYPE_LUN

        def check_usage_type(lun):
            # Only traditional lun can be managed
            return lun['USAGETYPE'] == constants.LUN_USAGE_TYPE_TRADITIONAL

        def check_has_rss_object(lun):
            if 'HASRSSOBJECT' not in lun:
                return True
            hasrssobject = json.loads(lun['HASRSSOBJECT'])
            hypermetro = hasrssobject.pop('HyperMetro', 'FALSE')
            if hypermetro.lower() == 'true' and not self.rmt_client:
                return False

            for key, value in hasrssobject.items():
                if value.lower() == "true":
                    LOG.debug("%s is enabled, manage is not supported", key)
                    return False
            return True

        funcs = (check_health_status, check_running_status, check_snapshot,
                 check_function_type, check_usage_type, check_has_rss_object)
        for func in funcs:
            if not func(lun):
                volume['status'] = fields.VolumeStatus.ERROR
                LOG.debug("LUN(%s) info check: %s failed",
                          lun["NAME"], func.__name__)
                break

    def _post_process_volume_info(self, volume_copy):
        """
        Additional information to be added to the volume before
        returning from query_volumes
        """
        super(HuaweiBaseDriver, self)._post_process_volume_info(volume_copy)
        LOG.info("Enter: _post_process_volume_info, volumes: %s", volume_copy)
        lun_id = volume_copy['provider_location']
        lun = self.client.get_lun_info(lun_id)

        provider_location_update = self._build_provider_location(lun)
        volume_copy.update(provider_location_update)

        replication_update = self._build_replication_info(lun)
        volume_copy.update(replication_update)
        LOG.info("Exit: _post_process_volume_info, volumes: %s", volume_copy)

    def _build_provider_location(self, lun):
        hypermetro_pair = self.client.get_hypermetro_by_lun_name(lun['NAME'])
        hypermetro_enabled = self._is_hypermetro_enabled(hypermetro_pair)
        provider_location = {
            'huawei_lun_id': lun['ID'],
            'huawei_sn': self.sn,
            'huawei_lun_wwn': lun['WWN'],
            'hypermetro': hypermetro_enabled
        }
        if hypermetro_enabled:
            provider_location['hypermetro_id'] = hypermetro_pair['ID']

        provider_location_json = huawei_utils.to_string(**provider_location)
        return {'provider_location': provider_location_json}

    def _is_hypermetro_enabled(self, hypermetro_pair):
        hypermetro_enabled = bool(hypermetro_pair)
        if not hypermetro_enabled:
            return False

        # Will raise error, but will not complaint in GUI
        if not self.rmt_client:
            msg = _("Volume has enabled HyperMetro but the remote client "
                    "does not config in backend")
            raise exception.VolumeBackendAPIException(data=msg)

        if hypermetro_pair.get("CGID"):
            msg = _("HyperMetro in consistency group cannot be managed")
            raise exception.VolumeBackendAPIException(data=msg)

        return True

    def _build_replication_info(self, lun):
        pair = self.client.get_pair_info_by_lun_id(lun['ID'])
        replication_enabled = bool(pair)
        if not replication_enabled:
            return {}

        msg = _("Volume with RemoteReplication is not supported")
        raise exception.VolumeBackendAPIException(data=msg)
    ...
    def check_for_deleted_volumes(self, context, volumes, tagged_vols=None):
        ...
        @return: A list of volumes that have not been found on the back end.
                 ...
                 is returned.
        """
        LOG.debug("Enter: check_for_deleted_volumes, volumes: %s, "
                  "tagged_vols: %s", volumes, tagged_vols)
        changed_vols = list()
        # nothing to do if we didn't get any volumes passed in. just return
        if len(volumes) == 0:
            return changed_vols
        for volume in volumes:
        ...
        wwns = self._get_all_lun_wwws()
        for volume in volumes:
            metadata = volume.get('volume_restricted_metadata', list())
            metadata = {m['key']: m['value'] for m in metadata}
            wwn = metadata.get(RESTRICTED_METADATA_VDISK_UID_KEY, "")
            if wwn.lower() not in wwns:
                self._handle_out_of_band_deleted(metadata, volume)
                changed_vols.append(volume)

        if changed_vols:
            names = [vol["name"] for vol in changed_vols]
            LOG.info("The following volumes do not exist: %s", names)

        return changed_vols

    def _handle_out_of_band_deleted(self, metadata, volume):
        metadata.update(out_of_band_deleted="True")
        self._update_restricted_metadata(volume['id'], metadata)

    def _get_all_lun_wwws(self):
        pools = self.client.get_inband_pools()
        return {lun['WWN'] for pool in pools for lun in
                self.client.get_all_luns(pool["ID"])}
...
class HuaweiISCSIDriver(HuaweiBaseDriver, driver.ISCSIDriver):
    ...
    def _initialize_connection(self, volume, connector, local=True):
        ...
        try:
            ...
            with excutils.save_and_reraise_exception():
                self.remove_host_with_check(host_id, client)
...
class HuaweiFCDriver(HuaweiBaseDriver, driver.FibreChannelDriver):
    ...
    def initialize_connection(self, volume, connector):
        ...
        fc_info = self._initialize_connection(volume, connector)
        if metadata.get('hypermetro'):
            rmt_fc_info = self._initialize_connection(volume, connector, False)
            fc_info = self._construct_fc_initialize_info(fc_info, rmt_fc_info)

        LOG.info("Return FC info is: %s.", fc_info)
        fczm_utils.add_fc_zone(fc_info)
        return fc_info

    def _get_hypermetro_rmt_lun_id(self, volume):
        hypermetro_id = huawei_utils.get_hypermetro_id(volume)
        if hypermetro_id:
            metro_info = self.rmt_client.get_hypermetro_by_id(hypermetro_id)
        else:
            lun_name = huawei_utils.get_lun_name(self.rmt_client, volume, True)
            metro_info = self.rmt_client.get_hypermetro_by_lun_name(lun_name)

        if not metro_info:
            msg = _('Volume %s is not in hypermetro pair') % volume.id
            LOG.error(msg)
            raise exception.VolumeBackendAPIException(data=msg)
        return metro_info['LOCALOBJID']

    def _initialize_connection(self, volume, connector, local=True):
        LOG.info('FC _initialize_connection, attach %(local)s volume.',
                 {'local': 'local' if local else 'remote'})
        # Determine use which client, local or remote.
        client = self.client if local else self.rmt_client
        ...
        if not local:
            lun_type = constants.LUN_TYPE
            lun_id = self._get_hypermetro_rmt_lun_id(volume)
        else:
            lun_id, lun_type = self.get_lun_id_and_type(
                volume, constants.VOLUME_NOT_EXISTS_RAISE)
        ...
        self_create_host_name = huawei_utils.encode_host_name(
            'O_s_' + connector['host'])
        host_id, exist_host_name = huawei_utils.get_exist_host(client, wwns)
        portg_id = None

        if not host_id:
            host_id = client.add_host_with_check(self_create_host_name)

        if not self.fcsan:
            self.fcsan = fczm_utils.create_lookup_service()

        if exist_host_name and exist_host_name != self_create_host_name:
            tgt_port_wwns, init_targ_map = client.get_init_targ_map(wwns)
        elif self.fcsan:
            # Use FC switch.
            zone_helper = fc_zone_helper.FCZoneHelper(self.fcsan, client)
            try:
            ...
            for ini in init_targ_map:
                client.ensure_fc_initiator_added(ini, host_id)
        else:
            # Not use FC switch.
            online_wwns_in_host = (
                client.get_host_online_fc_initiators(host_id))
            online_free_wwns = client.get_online_free_wwns()
            fc_initiators_on_array = client.get_fc_initiator_on_array()
            ...
            for wwn in wwns:
                if (wwn not in online_wwns_in_host
                    ...
                    wwns_in_host = client.get_host_fc_initiators(host_id)
                    iqns_in_host = client.get_host_iscsi_initiators(host_id)
                    if not (wwns_in_host or iqns_in_host or
                            client.is_host_associated_to_hostgroup(host_id)):
                        client.remove_host(host_id)
            ...
            for wwn in wwns:
                client.ensure_fc_initiator_added(wwn, host_id)

            (tgt_port_wwns, init_targ_map) = (
                client.get_init_targ_map(wwns))
        ...
        map_info = client.do_mapping(lun_id, hostgroup_id,
                                     host_id, portg_id,
                                     lun_type, hypermetro_lun)
        host_lun_id = client.get_host_lun_id(host_id, lun_id,
                                             lun_type)
        ...
        fc_info = {'driver_volume_type': 'fibre_channel',
                   'data': {'target_lun': int(host_lun_id),
                            ...
                            'libvirt_iscsi_use_ultrapath':
                                self.use_ultrapath}, }
        if init_targ_map:
            fc_info['data']['initiator_target_map'] = init_targ_map

        LOG.info("Return %(local)s FC info is: %(info)s." %
                 {'local': 'local' if local else 'remote', "info": fc_info})
        return fc_info

    def _construct_fc_initialize_info(self, loc_fc_info, rmt_fc_info):
        loc_tgt_wwn = loc_fc_info['data']['target_wwn']
        rmt_tgt_wwn = rmt_fc_info['data']['target_wwn']

        local_ini_tgt_map = loc_fc_info['data']['initiator_target_map'] \
            if loc_fc_info['data'].get('initiator_target_map') else {}
        rmt_ini_tgt_map = rmt_fc_info['data']['initiator_target_map'] \
            if rmt_fc_info['data'].get('initiator_target_map') else {}

        loc_fc_info['data']['target_wwn'] = (loc_tgt_wwn + rmt_tgt_wwn)
        for k in rmt_ini_tgt_map:
            local_ini_tgt_map[k] = (local_ini_tgt_map.get(k, []) +
                                    rmt_ini_tgt_map[k])

        loc_map_info = loc_fc_info['data']['map_info']
        ...
        loc_fc_info['data']['target_lun'] = same_host_id
    ...
    def terminate_connection(self, volume, connector, **kwargs):
        ...
        fc_info = self._terminate_connection(volume, connector)
        if metadata.get('hypermetro'):
            rmt_fc_info = self._terminate_connection(volume, connector, False)
            if self.fcsan:
                fc_info = self._construct_fc_terminate_info(
                    fc_info, rmt_fc_info)
    ...
    def _construct_fc_terminate_info(self, loc_fc_info, rmt_fc_info):
        local_ini_tgt_map = loc_fc_info['data']['initiator_target_map'] \
            if loc_fc_info['data'].get('initiator_target_map') else {}
        rmt_ini_tgt_map = rmt_fc_info['data']['initiator_target_map'] \
            if rmt_fc_info['data'].get('initiator_target_map') else {}
    ...
    def _terminate_connection(self, volume, connector, local=True):
        LOG.info('FC _terminate_connection, detach %(local)s volume.',
                 {'local': 'local' if local else 'remote'})
        # Determine use which client, local or remote.
        client = self.client if local else self.rmt_client
        ...
        self_create_host_name = huawei_utils.encode_host_name(
            'O_s_' + connector['host'])
        host_id, host_name = huawei_utils.get_exist_host(client, wwns)
        if not host_id:
            host_id = huawei_utils.get_host_id(client,
                                               self_create_host_name)
            is_self_create_host = True
        else:
            is_self_create_host = host_name == self_create_host_name
        ...
        if lun_id and lungroup_id:
            lungroup_ids = client.get_lungroupids_by_lunid(lun_id,
                                                           lun_type)
            if lungroup_id in lungroup_ids:
                client.remove_lun_from_lungroup(lungroup_id,
                                                lun_id,
                                                lun_type)
            else:
                LOG.warning("LUN is not in lungroup. "
        ...
        if int(left_lunnum) > 0:
            ...
            fc_info, portg_id = self._delete_zone_and_remove_fc_initiators(
                wwns, host_id, is_self_create_host, client)
            if lungroup_id:
                if view_id and client.lungroup_associated(
                        view_id, lungroup_id):
                    client.delete_lungroup_mapping_view(view_id,
                                                        lungroup_id)
                client.delete_lungroup(lungroup_id)
            if portg_id:
                if view_id and client.is_portgroup_associated_to_view(
                        view_id, portg_id):
                    client.delete_portgroup_mapping_view(view_id,
                                                         portg_id)
                    client.delete_portgroup(portg_id)

            if host_id:
                hostgroup_name = constants.HOSTGROUP_PREFIX + host_id
                hostgroup_id = client.find_hostgroup(hostgroup_name)
                if hostgroup_id:
                    if view_id and client.hostgroup_associated(
                            view_id, hostgroup_id):
                        client.delete_hostgoup_mapping_view(
                            view_id, hostgroup_id)
                    client.remove_host_from_hostgroup(
                        hostgroup_id, host_id)
                    client.delete_hostgroup(hostgroup_id)

                if not client.check_fc_initiators_exist_in_host(
                        host_id):
                    client.remove_host(host_id)
    ...
    def _delete_zone_and_remove_fc_initiators(self, wwns, host_id,
        ...
        if self.fcsan:
            zone_helper = fc_zone_helper.FCZoneHelper(self.fcsan, client)
            portg_id, init_targ_map = zone_helper.get_init_targ_map(wwns,
                                                                    host_id)

        # Remove the initiators from host if need.
        if host_id and is_self_create_host:
            fc_initiators = client.get_host_fc_initiators(host_id)
            for wwn in wwns:
                if wwn in fc_initiators:
                    client.remove_fc_from_host(wwn)
